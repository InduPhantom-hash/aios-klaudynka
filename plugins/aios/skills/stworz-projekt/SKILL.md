...
- `<nazwa>` - nazwa projektu (kebab-case preferowane, np. `moja-apka`)
- `<kategoria>` - jedna z kategorii zdefiniowanych w `me.md` (domyslnie: `Praca`, `Hobby`,
  `Prywatne`, `Vibe-coding` - mozesz miec wlasne)
...
1. Czy `<kategoria>` nalezy do kategorii zdefiniowanych w `me.md`
   (domyslnie `{Praca, Hobby, Prywatne, Vibe-coding}`)? Jesli nie - zatrzymaj, pytaj.
